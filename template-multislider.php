<?php
/**
 * Template Name: Home Page Multiple Slider
 *
...
function maggie_lite_web_layout_c($classes){
	$classes[] = "home";
	return $classes;
}    
add_filter( 'body_class', 'maggie_lite_web_layout_c' );
get_header();
$slider_type_option = 'multiple';//get_theme_mod('slider_type_option','single');
?>

<section class="slider-wrapper <?php echo $slider_type_option.'-slider-wrap';?>">
    <?php

    $slider_category = 'uncategorized';//get_theme_mod('homepage_option_slider');
  ...
  if($slider_query->have_posts())
  {
    ...
    while($slider_query->have_posts())
    {
      ...
      if($slider_type_option=='multiple'){
        $post_big_image_path = wp_get_attachment_image_src( $post_image_id, 'maggie-lite-block-small-thumb', true );
      }else{
        $post_big_image_path = wp_get_attachment_image_src( $post_image_id, 'maggie-lite-slider-image', true );
      }
      ...
      <div class="slider">
        <div class="slide-image">
          <img src="<?php echo esc_url( $post_big_image_path[0] );?>" alt="<?php echo esc_attr($post_image_alt);?>" />
          ...
          <div class="mag-slider-caption">
            <div class="maggie-container">
              <div class="maggie-slider-caption-wrap">
                ...
                <a class="slider-btn" href="<?php echo the_permalink();?>"><?php esc_html_e( 'READ MORE', 'maggie-lite' ); ?></a>
              </div>
            </div>
          </div>
        </div>
...
if(get_theme_mod('feature_post_zero','')!=""){
    ?>
    <section class="below-slider-feature wow fadeInUp clearfix" data-wow-delay="0.5s">
        <div class="maggie-container">
            <?php 
            ...
            if($category_info!=null){
                ...
                if($block0_query->have_posts()){
                    while($block0_query->have_posts()){
                        ...
                        $b0_image_id = get_post_thumbnail_id();
                        $b0_big_image_path = wp_get_attachment_image_src($b0_image_id,'maggie-lite-featured-post',true);
                        $b0_image_alt = get_post_meta($b0_image_id,'_wp_attachment_image_alt',true);
                        ?>                        
                            ...
                            <?php if(has_post_thumbnail()): ?> 
                                <div class="post-image">
                                    ...
                                    <a href="<?php the_permalink();?>">
                                        <img src="<?php echo esc_url( $b0_big_image_path[0] );?>" alt="<?php echo esc_attr($b0_image_alt);?>" />
                                    </a>
                                </div>                                
                            <?php endif ; ?>
            ...
            if($category_info!=null){
                ...
                if($block1_query->have_posts()){
                    while($block1_query->have_posts()){
                        ...
                        $b1_big_image_path = wp_get_attachment_image_src($b1_image_id,'maggie-lite-block-big-thumb',true);
                        $b1_smallright_image_path = wp_get_attachment_image_src($b1_image_id,'maggie-lite-block-smallright-thumb',true);
                        $b1_small_image_path = wp_get_attachment_image_src($b1_image_id,'maggie-lite-block-small-thumb',true);
                        ...
                        <div class="single_post <?php if($b_counter == 1){echo 'top-post non-zoomin';}?>">
                            <?php if(has_post_thumbnail()): ?>   
                                <div class="post-image">
                                    ...
                                    <a href="<?php the_permalink();?>">
                                        <img src="<?php if($b_counter ==1){echo esc_url( $b1_big_image_path[0] );}elseif($b_counter<5){ echo esc_url( $b1_smallright_image_path[0] ) ;}else{ echo esc_url( $b1_small_image_path[0] ) ;}?>" alt="<?php echo esc_attr($b1_image_alt);?>" />
                                    </a>
                                    <?php do_action( 'maggie_lite_post_format_icon' ); ?>
                                </div>                                
                            <?php endif ; ?>
            ...
            if($category_info_2!=null){
                ...
                if($block2_query->have_posts()){
                    while($block2_query->have_posts()){
                        ...
                        $b2_big_image_path = wp_get_attachment_image_src($b2_image_id,'maggie-block-big-thumb',true);
                        $b2_small_image_path = wp_get_attachment_image_src($b2_image_id,'maggie-lite-block-small-thumb',true);
                        ...
                        <div class="single_post">
                            <?php if(has_post_thumbnail()): ?>   
                                <div class="post-image">
                                    ...
                                    <a href="<?php the_permalink();?>">
                                        <img src="<?php echo esc_url( $b2_small_image_path[0]);?>" alt="<?php echo esc_attr($b2_image_alt);?>" />
                                    </a>
                                    <?php do_action( 'maggie_lite_post_format_icon' ); ?>
                                </div>                                
                        ...
                        <div class="single_post clearfix <?php if($b_counter == 1){echo 'top-post non-zoomin';}elseif($b_counter==2){echo "second-post";}?>">
                            ...
                            if($b_counter<3){
                                ...
                                <div class="post-image">
                                    ...
                                    <a href="<?php the_permalink();?>">
                                        <img src="<?php if($b_counter ==1){echo esc_url( $b3_big_image_path[0] );}else{ echo esc_url( $b3_small_image_path[0] ) ;}?>" alt="<?php echo esc_attr($b3_image_alt);?>" />
                                    </a>
                                    <?php do_action( 'maggie_lite_post_format_icon' ); ?>
                                </div>                               
                                <?php
        ...
        if($category_info_4!=null){
            ...
            if($block4_query->have_posts()){
                while($block4_query->have_posts()){
                    ...
                    $b4_big_image_path = wp_get_attachment_image_src($b4_image_id,'maggie-lite-blockfour-thumb',true);
